...
class DB{
	...
	function create_table_column_string($column_arr){
		$return_string="id int PRIMARY KEY AUTO_INCREMENT,";
	...
	function db_table_create_table($table){
		$tableObj=new Table;
		$tableObj=$table;
		$this->db_exec("CREATE TABLE ".$tableObj->table_name." ( ".$this->create_table_column_string($tableObj->column_arr)." );");
	...
	function db_tab_create_tab($tab){
		$tabObj=new Tab;
		$tabObj=$tab;
		$hide_array=array(":tab_name"=>$tabObj->tab_name, ":tab_type"=>$tabObj->tab_type, ":id"=>$tabObj->layout_id);
		$this->db_exec_esc("INSERT INTO DBTab (id_tab, id_layout, tab_name, tab_type, xml, sqljoins) VALUES (NULL, :id, :tab_name, :tab_type, '','');", $hide_array);
	}
